...
class LearnThBlock(nn.Module):
    def __init__(self, fc_weights, fc_bias, num_classes):
        super(LearnThBlock, self).__init__()
        ksize = (3, 3, 3)
        pad = (1, 1, 1)
        nt_stride = (1, 2, 2)
        stride = (2, 2, 2)
...
class EarlyExitBlockC_light_weight(nn.Module): ##input to the layer (batch,512,4,7,7)
    def __init__(self,fc_weights,fc_bias, num_classes):
        ...
        no_stride =(1,1,1)
        ######Threshold learning############
        self.th_avg_pool = nn.AvgPool3d((4, 14, 14), stride=1)
        self.th_max_pool = nn.MaxPool3d((4, 14, 14), stride=1)
        self.ch_down = nn.Linear(512, 256)
        self.fc2 = nn.Linear(256, 128)
        self.fc1 = nn.Linear(128, 32)
        self.fc = nn.Linear(32, 1)
        #####################################
        self.conv1_exit0 = MF_UNIT(512, 256, 256,  stride=no_stride, g=32)
        self.conv2_exit0 = MF_UNIT(256, 256, 2048,  stride=stride, g=16)
        #model.first_layer.weight.data
        self.fc_exit0 = nn.Linear(2048, num_classes)
    ...
    def forward(self, x):
        #########################################
        th_avg_pool = self.th_avg_pool(x)     ##out:(batch,512) or (batch,512,1,1)
        th_avg_pool = torch.squeeze(th_avg_pool)
        th_max_pool = self.th_max_pool(x)     ##out:(batch,512) or (batch,512,1,1)
        th_max_pool = torch.squeeze(th_max_pool)

        stat = th_avg_pool + th_max_pool
        if len(stat.shape) == 1: ###batch == 1 and gone doing squeeze
            stat= torch.unsqueeze(stat,0)
        stat = self.ch_down(stat)##out: (batch,256)
        #########################################
        exit0 = self.conv1_exit0(x)
        #########################################
        th_avg_pool1 = self.th_avg_pool(exit0)  ##(batch,256) or (batch,256,1,1)
        th_avg_pool1 = torch.squeeze(th_avg_pool1)
        th_max_pool1 = self.th_max_pool(exit0)  ##(batch,256) or (batch,256,1,1)
        th_max_pool1 = torch.squeeze(th_max_pool1)
        stat1 = th_avg_pool1 + th_max_pool1
        if len(stat.shape) == 1:   ###batch == 1 and gone doing squeeze
            stat1= torch.unsqueeze(stat1,0)
        stat = stat + stat1
        threshold =self.fc(self.fc1(self.fc2(stat)))
        #########################################
        ...
        exit0 = torch.squeeze(exit0)
        exit0 = self.fc_exit0(exit0)
        return exit0, threshold



class EarlyExitBlockC_light_weight_nopool(nn.Module): ##input to the layer (batch,512,4,7,7)
    def __init__(self,fc_weights,fc_bias, num_classes):
        super(EarlyExitBlockC_light_weight, self).__init__()
        ksize=(3,3,3)
        pad=(1,1,1)
        ...
        self.th_avg_pool = nn.AvgPool3d((4, 14, 14), stride=1)
        self.th_max_pool = nn.MaxPool3d((4, 14, 14), stride=1)
        self.conv_th1 = Conv3dEE(512, 128,kernel_size=(7,7,7), padding=(3,3,3), stride=(2,4,4), g=16, bias=False)
        self.conv_th2 = Conv3dEE(128, 1,kernel_size=(3,3,3), padding=(1,1,1), stride=(2,2,2), g=4, bias=False)
    ...
    def forward(self, x):
        #########################################
        threshold = self.conv_th1(x)
        threshold = self.conv_th2(threshold)
        #########################################
        exit0 = self.conv1_exit0(x)
        #########################################
...
#        self.exit2 = EarlyExitBlockC(fc_weights,fc_bias,num_classes)
    ...
    def forward(self, x):
        ...
        ##########exit2##############        
        exit2, threshold = self.exit2(x)
        if (len(exit2.shape) == 1):
            exit2 = torch.unsqueeze(exit2,0)
        output.append(exit2)
...
def resnet50(**kwargs):
    ...
    """
    model = ResNextEE(ResNeXtBottleneck, [3, 4, 6, 3], **kwargs)
    return model


def resnext101(fc_weights,fc_bias,num_classes,frame_size,frames_sequence,**kwargs):
    """Constructs a ResNet-101 model.
    """
   # print("inside calling func, number of classes:",num_classes)
    model = ResNextEE(fc_weights,fc_bias,ResNeXtBottleneck, [3, 4, 23, 3],num_classes,frame_size,frames_sequence)
    return model
...
def resnet152(**kwargs):
    """Constructs a ResNet-101 model.
    """
    model = ResNextEE(ResNeXtBottleneck, [3, 8, 36, 3], **kwargs)
    return model
...
if __name__ == "__main__":
    ...
    # ---------
    net = ResNextEE(0,0,ResNeXtBottleneck, [3, 4, 23, 3],num_classes=51)
