    ...
    for i in range(batch_size):
        for exit in range(opt.num_exits):
           ...
           # print("exit_num:",exit)
            ...
            if binary_taken[i*(opt.num_exits-1) + exit]:##binary_taken is [batch1_exit0,batch1_exit1,batch2_exit0 batch2_exit1..... ]
                #print("outputs[i][exit]",outputs[exit][i].shape)
                _, pred = outputs[exit][i].topk(1, 0, True)
