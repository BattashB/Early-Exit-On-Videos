...
class MF_UNIT(nn.Module):
    ...
    def __init__(self, in_channels, num_mid, out_channels, g=1, stride=(1,1,1),change_channels=False):
    ...
    def forward(self, x):
       ...
       # print("x is:",x.shape,"h is:",h.shape)
...
class EarlyExitBlockC(nn.Module):
    def __init__(self,fc_weights,fc_bias, num_classes=400):
        ...
        self.adjust_channels_conv = Conv3dEE(in_channels, out_channels,kernel_size=(1,1,1), padding=(0,0,0), stride=(1,1,1), g=1, bias=False)
        self.conv1_exit0 = MF_UNIT(out_channels, out_channels,out_channels,  stride=no_stride, g=32)
        self.conv2_exit0 = MF_UNIT(out_channels, out_channels,256,  stride=no_stride, g=32,change_channels=change_channels)
        self.conv3_exit0 = MF_UNIT(256, 256,256,  stride=no_stride, g=32)
        self.conv4_exit0 = MF_UNIT(256, 256,2048,  stride=stride, g=16,change_channels=change_channels)
    ...
    def forward(self, x):
        #print("input x:",x.shape)
        exit0 = self.adjust_channels_conv(x)
        #print("conv1 now!!")
...
def resnext101(fc_weights,fc_bias,opt,num_classes,frame_size,frames_sequence,**kwargs):
    ...
    """
    model = ResNextEE(fc_weights,fc_bias,opt,ResNeXtBottleneck, [3, 4, 23, 3],frame_size,frames_sequence, **kwargs)
    return model
