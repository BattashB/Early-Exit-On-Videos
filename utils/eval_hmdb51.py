...
def compute_video_hit_at_k(ground_truth, prediction, top_k=3):
    ...
    for i, vid in enumerate(video_ids):
        ...
        if not pred_idx.any():
            list_of_vid_not_in_pred.append(vid)
            notfound+=1
            continue
    ...
    print(list_of_vid_not_in_pred)
    return float(avg_hits_per_vid.mean())
    
    
if __name__ == "__main__":
    hmdb = HMDBclassification('/workspace/Early-Exit-In-Videos/annotation_dir_path/hmdb51/hmdb51_2.json', 'results/val1.json',verbose=True, top_k=1) 
    hmdb.evaluate() 
