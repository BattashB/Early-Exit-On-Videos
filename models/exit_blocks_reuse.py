...
def mish(input):
    if torch.cuda.is_available():
        out = input * torch.tanh(F.softplus(input))
        return out          
...
class EarlyExitBlockBS_LR_E2(nn.Module):
    def __init__(self,block, layers, shortcut_type, cardinality,sample_size,frames_sequence,inplanes_block1,inplanes_block2, num_classes=51):
        ...
        self.dropout = nn.Dropout(0.5)
        self.fc = nn.Linear(cardinality * 32 * block.expansion, num_classes)
        self.downconv1 = nn.Sequential(nn.Conv3d(2048,1024,kernel_size=1,padding=0),nn.BatchNorm3d(1024))
        #torch.catdownx = nn.Sequential(nn.Conv3d(kernel_size=1,stride=stride,groups=16, bias=False), nn.BatchNorm3d())
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                m.weight = nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            elif isinstance(m, nn.BatchNorm3d):
    ...
    def forward(self, x,exit1_a,exit1_b):
        out = []
        #exit1_a.shape[b, 1024, 2, 7, 7],exit1_b.shape[b, 2048, 1, 4, 4]
        #x = torch.cat([x,exit1_a],dim=1)
        exit2 = self.block1(x) 

        exit2 = torch.cat([exit2,exit1_a],dim=1)
        exit2 = mish(self.downconv1(exit2))
        out.append(exit2)
        
        exit2 = self.block2(exit2) 
        out.append(exit2)
        exit2 = self.avgpool(exit2)
        exit2 = torch.squeeze(exit2)
        exit2 = self.dropout(exit2)
        exit2 = self.fc(exit2)
        out.append(exit2)
...
class EarlyExitBlockBS(nn.Module):
    def __init__(self,block, layers, shortcut_type, cardinality,sample_size,frames_sequence,inplanes_block1,inplanes_block2, num_classes=51):
        ...
        #downconv1 = nn.Sequential(nn.Conv3d(2048,1024,kernel_size=1,padding=0),nn.BatchNorm3d(1024))
    def _make_layer(self,
                    block,
                    planes,
                    blocks,
