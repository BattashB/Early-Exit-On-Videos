...
class DeformableConv(nn.Module):
    def __init__(self, in_channels,out_channels,kW,kH):
        super(DeformableConv, self).__init__()
        num_deformable_groups = 1
        self.conv_pred_offsets = nn.Conv2d(
            in_channels,
    ...
    def forward(self, x):
        offset = self.conv_pred_offsets(x)
...
class MF_UNIT(nn.Module):
    ...
    def __init__(self, in_channels, num_mid, out_channels, g=1, stride=(1,1,1)):
        ...
        self.deform  = DeformableConv(in_channels,num_ix,3,3)
        ...
        #self.conv_time = Conv3dEE(in_channels=in_channels,  out_channels=out_channels, kernel_size=(1,1,1), padding=(0,0,0), stride=stride)

        
        
    def forward(self, x):
        z = torch.chunk(x, x.shape[2], dim=2) 
        out_deform = []
        for i in range(x.shape[2]):#spliting time domain 
            out_deform.append(self.deform(torch.squeeze(z[i])))
        h = torch.stack(out_deform, dim=2, out=None) ### bring it back to 5d shape
        #h = self.conv_i1(x)
...
class MF_UNIT(nn.Module):
    ...
    def __init__(self, in_channels, num_mid, out_channels, g=1, stride=(1,1,1)):
        ...
        #self.conv_i1 =     Conv3dEE(in_channels=in_channels,  out_channels=num_ix,  kernel_size=(1,1,1), padding=(0,0,0))
        self.conv_i2 =     Conv3dEE(in_channels=num_ix,  out_channels=in_channels,  kernel_size=(1,1,1), padding=(0,0,0))
        self.conv_m1 =     Conv3dEE(in_channels=in_channels,  out_channels=num_ix, kernel_size=(3,3,3), padding=(1,1,1), stride=stride, g=g)
        self.conv_m2 =     Conv3dEE(in_channels=num_ix, out_channels=out_channels, kernel_size=(1,3,3), padding=(0,1,1), g=g)
        if self.w1conv:
            self.conv_w1 = Conv3dEE(in_channels=in_channels,  out_channels=out_channels, kernel_size=(1,1,1), padding=(0,0,0), stride=stride)
        #self.conv_time = Conv3dEE(in_channels=in_channels,  out_channels=out_channels, kernel_size=(1,1,1), padding=(0,0,0), stride=stride)
...
class EarlyExitBlockB_light_weight(nn.Module):
    def __init__(self,fc_weights,fc_bias, num_classes):
        super(EarlyExitBlockB_light_weight, self).__init__()
        ksize=(3,3,3)
        pad=(1,1,1)
        nt_stride =(1,2,2)
        stride = (2,2,2)
        no_stride =(1,1,1)
        self.conv1_exit0 = MF_UNIT(256, 128,128,  stride=stride, g=32)
        self.conv4_exit0 = MF_UNIT(128, 128,2048,  stride=stride, g=32)
...
class EarlyExitBlockC_light_weight(nn.Module):
    def __init__(self,fc_weights,fc_bias, num_classes):
        ...
        self.conv4_exit0 = MF_UNIT(256, 256,2048,  stride=stride, g=32)
        #model.first_layer.weight.data
        self.fc_exit0 = nn.Linear(2048, num_classes)
...
class ResNextEE(nn.Module):
    ...
    def __init__(self, fc_weights,fc_bias,
        ...
        self.exit1 = EarlyExitBlockB_light_weight(fc_weights,fc_bias,num_classes)
